...
HANDLE console = GetStdHandle(STD_OUTPUT_HANDLE);
 void *x = enterData(4, 1, 0, console,4);
 printf("\n  %d - %s",x,x);
    return 0;
}
...
void load(HANDLE console)
{
    ...
    gotoxy(xpos,++tempypos);
    char *filename = enterData(FILE_NAME_SIZE, 2, tempypos, console);
    f = fopen(filename,"r");
    free(filename);
    char temp[20],*token;
    if(f!= NULL)
...
void Search(HANDLE console)
{
    ...
    gotoxy(xpos,++tempypos);
    char *input = enterData(LAST_NAME_SIZE, 2, tempypos, console);
    int check = 0;
    x = strlen(input);
...
void Add(int f, HANDLE console)
{
    ...
    printf("First name: ");
    gotoxy(xpos,++tempypos);
    input = enterData(FIRST_NAME_SIZE, 2, tempypos, console);
    ...
    printf("Last name: ");
    gotoxy(xpos,++tempypos);
    input = enterData(LAST_NAME_SIZE, 2, tempypos, console);
    ...
    printf("Address: ");
    gotoxy(xpos,++tempypos);
    input = enterData(ADDRESS_SIZE, 2, tempypos, console);
    ...
    while(check == 0)
    {
        ...
        printf("E-mail : ");
        gotoxy(xpos,++tempypos);
        input = enterData(EMAIL_SIZE, 2, tempypos, console);
